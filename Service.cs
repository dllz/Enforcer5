...
namespace Enforcer5
{
    public static class Service
    {
        ...
        {
            var type = Redis.db.HashGetAsync($"chat:{message.Chat.Id}:welcome", "type");
            var content = Redis.db.HashGetAsync($"chat:{message.Chat.Id}:welcome", "content");
            ...
            {
                var hasMedia = Redis.db.HashGetAsync($"chat:{message.Chat.Id}:welcome", "hasMedia");
                if (!string.IsNullOrEmpty(hasMedia) && hasMedia.Equals("true"))
                {
                    var file = Redis.db.HashGetAsync($"chat:{message.Chat.Id}:welcome", "media");
        ...
        {
            var groupBan = Redis.db.HashGetAsync($"groupBan:{updateMessage.Chat.Id}", "banned");
            ...
            var alreadyExists = Redis.db.SetContainsAsync($"bot:groupsid", updateMessage.Chat.Id);
            if (alreadyExists)
            {
                await Bot.Send(
