...
namespace Enforcer5.Helpers
{
    public class Methods
    {
        ...
        {
            var lang = Redis.db.StringGetAsync($"chat:{uMessage.Chat.Id}:language");
            if (lang.HasValue)
            {
        ...
        {
            var lang = Redis.db.StringGetAsync($"chat:{chatId}:language");
            if (lang.HasValue)
            {
                return Program.LangaugeList.FirstOrDefault(x => x.Name == lang);
            }
        ...
        public static int ResolveIdFromusername(string s, long chatId = 0)
        {
            if (chatId != 0)
            {
               var userid =  Redis.db.HashGetAsync($"bot:usernames:{chatId}", s);
                var id = 0;
            ...
            {
                var userid = Redis.db.HashGetAsync($"bot:usernames", s);
                var id = 0;
                if (int.TryParse(userid, out id))
                {
                    return id;
        ...
        {
            var about = Redis.db.StringGetAsync($"chat:{chatId}:about");
            if (about.HasValue)
            {
        ...
        {
            var rules = Redis.db.StringGetAsync($"chat:{chatId}:about");
            if (rules.HasValue)
            {
        ...
        public static string GetUserInfo(int userid, long? chatId, string chatTitle, XDocument lang)
        {
            ...
            var hash = $"ban:{userid}";
            var banInfo = Redis.db.HashGetAllAsync(hash);
        ...
        {
            var isBanned = Redis.db.HashGetAllAsync($"globanBan:{update.Message.From.Id}");
            try
            {
        ...
        {
            var tempbans = Redis.db.HashGetAllAsync("tempbanned");
