...
namespace Enforcer5
{
    public static partial class Commands
    {
        ...
        {
            var num = Redis.db.HashIncrementAsync($"chat:{update.Message.Chat.Id}:warns", update.Message.From.Id, 1);
            var max = 3;
            int.TryParse(Redis.db.HashGetAsync($"chat:{update.Message.Chat.Id}:warnsettings", "max"), out max);
            var lang = Methods.GetGroupLanguage(update.Message);
            if (num >= max)
            {                
                var type = Redis.db.HashGetAsync($"chat:{update.Message.Chat.Id}:warnsettings", "type").HasValue
